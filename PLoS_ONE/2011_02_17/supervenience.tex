...
\newtheorem{thex}{\emph{Gedanken} experiment}
...
The supervenience conjecture implies that%, the above mind brain supervenience conjecture may be formally stated
...
% \begin{align}
% \EE[L_{\PP}(g_n)] 
% &\approx  \hL^{n'}_{\PP}(g_n) = \sum_{\mc{T}_{\mt{n}}} \PP[g_n(B)=M | \mc{T}_{\mt{n}}] \PP[\mc{T}_{\mt{n}}],
% \end{align}
...
%, and the sum is taken over a sufficiently large number of subsets, $\mc{T}_{\mt{n}}$, such that $\hL^{n'}_{\PP}(g_n)$ converges.  
 % $n' \hL^{n'}_{\PP}(g_n)$ is t
The approximate  number of misclassified minds
%. Simplifying further by assuming just one hold-out set, then $n' \hL^{n'}_{\PP}(g_n)$ 
therefore
% and 
has a binomial distribution:  $n' \hL^{n'}_{\PP}(g_{\mt{n}}) \sim \text{Binomial}(n',L_{\PP}(g_{\mt{n}}))$. %, because for any of the $n'$ held-out samples, the classifier could be either correct or incorrect.
...
\section{A \emph{gedanken} experiment demonstrating consistency and unity power} % (fold)
\label{sub:uc}
...
\subsection{Alternative explanations} % (fold)

Although hypothesis testing for a particular $\epsilon$-supervenience appears to be possible based on the \emph{gedanken} experiment in Section \ref{sub:uc}, a natural question to raise is: what are the conceivable alternative hypotheses?  We consider three such alternatives.  

...
\begin{align}
\EE[L_{\PP}(g_n)] %= \int \PP[g_n(B)=M | \mc{T}_{n'-n}] \PP[\mc{T}_{n'-n}] d\mc{T}_{n'n} \nonumber \\ 
...
% Thus, while the above Bayes plugin estimators are sufficient in some scenarios, they are not a panacea.  In particular, the Bayes plugin classifier suffers by having slow convergence properties.  More specifically, it lacks the ability to generalize, in that any graph not previously observed will be classified according to the prior, no matter how ``close'' it is to other previously observed graphs.  A different algorithm called the $k_n$ nearest neighbor (knn) algorithm, can significantly expedite convergence by defining a dissimilarity metric, $d(\cdot,\cdot): \mc{B} \times \mc{B} \mapsto \Real$,  which effectively assumes a notion of proximity.  Although originally proven to be universally consistent when observations are vector-valued random graphs, by embedding the graphs into a vector space, the result holds true for graph data as well (see the appendix for details).  Using $k$-NN algorithm therefore can classify previously unseen graphs based on their proximity to graphs in the training set.



% subsection brain_graphs (end)

\subsection{Theoretical implications} % (fold)
...
% Further, collecting enough sufficiently accurate, exchangeable brain-graph data and the associated mental property values may be beyond current technological capabilities. 

 % (see Appendix 2 for a simulated example of a feasible experiment). Nevertheless, our thought experiment suggests that we can hope to determine that a given mental property under consideration $\varepsilon$-supervenes on a brain's connectivity structure. 

% Practical issues regarding actually conducting the above thought experiment include: (1) as stated, we must consider the space $\mB$ to be the quotient space of graphs mod graph isomorphism, unless the vertices are {\it labeled}; (2) 

% paragraph practical_issues (end)


%A simulation of a realistically performable experiment in C. elegans suggests that convergence rates are not so poor as to make these results completely impractical.  Several points merit further discussion, in our humble opinion.  
...
% Related experimental work includes collecting various types of brain graph data \cite{WhiteBrenner86, DenkHorstmann04, BriggmanDenk06} and various approaches to inference on brain graphs \cite{MackeBorst08, Mishchenko09, LuLichtman09}, suggests feasibility of such an experiment in the future.
...
% Another algorithm, proven to be universally consistent, is the $k_n$ nearest neighbor algorithm \cite{Stone77}, can rectify these limitations of the Bayes plugin algorithms.  

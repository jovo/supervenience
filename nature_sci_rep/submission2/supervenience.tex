...
Although the above theorem is of potential theoretical interest, the argument relies on knowing the typically unknown $F_{M,B}$, rendering it useless pragmatically.  However, either $F_{M,B}$ or $g^*$ could be estimated from data.  Before explicitly considering the problem of testing for statistical supervenience, we define a relaxed notion of supervenience:
\begin{defi}
\label{def2}
...
Assuming for the moment that the space of all possible minds is finite, that is, $|\mM| < \infty$, then we call any function, $g: \mc{B} \mapsto \mc{M}$, a \emph{classifier} (this assumption will later be relaxed).  Let $\mh{m}$ denote the output of a classifier, $g(b)=\mh{m}$.  Define misclassification rate as $L_{F}(g) = \PP[g(B) \neq M]$ which denotes the probability that $g$ misclassifies $b$ under distribution $F$. The Bayes optimal classifier $g^*$ minimizes $L_{F}(g)$ over all classifiers, that is:	
$g^* = \argmin_{g} L_{F}(g)$.
Thus, the \emph{Bayes error}, or Bayes risk, $L_{F}(g^*)$, is the minimum possible misclassification rate.

Let $\mc{T}_n=\{(m_1,b_1), (m_2,b_2), \ldots, (m_n,b_n)\}$ be a set of random samples taking their values in $\mc{M} \times \mc{B}$, each independently and identically distributed according to $F_{M,B}$.  An \emph{induced} classifier, $g_n:\mB \times (\mc{M} \times \mc{B})^n \mapsto \mM$, takes as input an observed brain $b$ and training data $\mc{T}_n$, and classifies: $g_n(b;\tr{\mc{T}_n})=\mh{m}$.  Misclassification rate for this classifier will be a random variable, because the training data $\mc{T}_n$ are random samples.  The expected misclassification rate for this classifier is therefore approximated by ``hold-out'' error:
$\hL^{n'}_{F}(g_{\mt{n}}) = \PP[g_{\mt{n}}(B)=M | \mc{T}_{\mt{n}}]$,
where $\mt{n}=n-n'$, and $n'<n$ is the number of held-out training samples (samples not used to obtain $g_{\mt{n}}$).
